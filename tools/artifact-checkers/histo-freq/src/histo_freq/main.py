# This file contains the function used to plot a histogram of frequencies for
...
def plot_histogram(
    ...
    """
    Generate a vertical (by default) or horizontal histogram graph and save it
    to a .html file.

    Args:
        list_of_buckets: List[List[float]]
                        a list of lists containing latencies (floats), wherein
                        each sub-list is a bucket.
        list_of_frequencies: List[int]
                            a list of frequencies (integers).
        file_name_wo_ext: str
                        The file_name_wo_ext to add to the histogram's file
                        name, e.g., 'READ-st' or 'WRITE-st',
                        preceding the file's extension.
        adjust_hist_bar_widths: bool
                            whether the histogram's bars' widths have to be
                            adjusted to reflect each bucket's range of
                            values (False by default as some ranges of
                            latencies may be too high with respect to
                            other ranges of latencies, and thus it is suggested
                            not to adjust them. However, this
                            parameter is included here to make it reusable for
                            other purposes in future if needed.).
        bar_gap: float
                the gap between the bars of the histogram.
        file_ext: str
                The file's extension. By default, it is '.html', so that the
                histogram graph is interactive, e.g.,
                            by hovering the mouse over the bars, the x and y
                            values corresponding to each bar can be
                            visualised interactively.
        is_vertical: bool
                    a boolean indicating whether the histogram should be
                    plotted vertically (if True, by default) or
    ...
    if adjust_hist_bar_widths:
        # Update the width of the bars in the histogram to reflect each
        # bucket's range of values.
        list_of_diffs = []
        for bucket in list_of_buckets:
            list_of_diffs.append(max(bucket) - min(bucket))
        fig.update_traces(width=list_of_diffs)

    # As suggested on GitHub at
    # https://github.com/plotly/plotly.py/issues/2876#issuecomment-1111339561,
    # the line below overwrites the default 'sum of' shown by plotly when
    # hovering the mouse on each bar
    # in the histogram, in this case with nothing (an empty string), as not
    # needed, so that the y-axis label
    # defined above is kept as is.
    fig.for_each_trace(
        lambda t: t.update(hovertemplate=t.hovertemplate.replace("sum of", ""))
    )

    # The filename and the .html extension are passed to export a html file
    # with the histogram graph on it.
...
def run():
    ...
    # After identifying the above-mentioned json files in their directory,
    # output .html files with the same file names
    # as above (i.e., WRITE-st.html and READ-st.html), but having the
    # corresponding interactive histogram graph

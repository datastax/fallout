# This file contains utility-type of functions to assist with the creation
# of a histogram graph, e.g., for preparing
...
def create_df_w_lats_ranges_and_freqs(
    ...
    """
    Create a dataframe of latency buckets' ranges and frequencies given two
    lists containing a list of buckets of
    ...
    Returns:
        a dataframe of latency buckets' ranges (strings)
        and frequencies (integers).
    """

    list_of_min_and_max = []
    for bucket in list_of_buckets:
        list_of_min_and_max.append(
            f"{str(round(min(bucket), PRECISION_VAL))}{'-'}"
            f"{str(round(max(bucket), PRECISION_VAL))}"
        )

...
public class InMemoryCsvCheck
{
    ...
    {
        csvSeparator = PropertySpecBuilder.create(prefix)
            .name("separator")
            .description("Separator to use to split the CSV columns")
            .defaultOf(DEFAULT_CSV_SEPARATOR)
            .parser(input -> input.toString().charAt(0))
            .validator(input -> input.toString().length() == 1)
            .build();
        sqlQueries = PropertySpecBuilder.create(prefix)
            .required()
            .name("query")
    ...
    {
        return Arrays.asList(csvSeparator, sqlQueries, expectedValue);
    }
